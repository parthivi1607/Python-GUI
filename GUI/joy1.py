...
class ThreadM(QThread):
            ...
            if(self.comm_sock.recv(1)=='j'):
                ...
                for i in range(24):
                    if(self.comm_sock.recv(1)=='w'):
                        ...
                        x=x*factor[i/2]
                        print round(x,2),"\t",
                        qwerty = qwerty + str(round(x,2)) + "\t"
                        if i==10:
                            qwerty = qwerty + "\t\t"

                self.signalbms.emit(qwerty)
    ...
    def sending(self):
        ...
        self.x_joy=str(int(self.x_joy)).zfill(5)
        self.y_joy=str(int(self.y_joy)).zfill(5)
        if self.hill_assist==True:
            hill='z'
        else:
            hill='m'
        val=hill+str(self.gear)+"s"+str(self.x_joy)+"f"+str(self.y_joy)
        self.comm_sock.sendall(val.encode())
        time.sleep(0.01)
    ...
    def wasd_publisher(self):
        ...
        if (not (keyinput[pygame.K_LCTRL] and keyinput[pygame.K_w]) and not (keyinput[pygame.K_LCTRL] and keyinput[pygame.K_a]) and (
            ...
            if self.sp_flag == 1:
                ob3.data = "{} {} {} {} {} {} {} {} {}".format('m','1','39','90','96','135','186','192','n')
            elif self.sp_flag == 0:
            ...
            try:
                ...
                while True:
                    ...
                    if self.mode == 's':
                        ...
                        self.bms_thread = threading.Thread(target=self.bmsval, args=())
